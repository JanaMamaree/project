...
$getorders = "select * from aid where donation='$type' and status='Confirmed!'";
$result = mysqli_query($con, $getorders);
if(mysqli_num_rows($result)== 0){
   echo "<br> <p align='center' style='font-size:20px'> no aid requests found </p>";
}else{
     

//$return_array = array();
echo '<br/><br/><table class="table">' . "\r\n";
echo "<tr><th scope='col'>Email</th><th scope='col'>City</th><th scope='col'>Street</th><th scope='col'>Type of donation</th><th scope='col'>Item</th><th scope='col'>Select to donate</th></tr>";


while ($row = mysqli_fetch_assoc($result)) {
?>
<tr><td scope="row"><?=$row['email']?></td><td scope="row"><?=$row['city']?></td><td scope="row"><?=$row['street']?></td><td scope="row"><?=$row['donation']?></td><td scope="row"><?=$row['item']?></td><td scope="row"><a href="donatenowswal.php?id=<?=$row['id']?>&email=<?=$row['email']?>&donation=<?=$row['donation']?>&item=<?=$row['item']?>"><button style="background-color: #c1e2b3;" type="button">Donate</button></a></td></tr>
