...
<link rel="stylesheet" type="text/css" href="css/sweetalert.css">
    ...
    swal({
  title: "Are you sure you want this item?",
  text: "Only you're first request will be taken into consideration!",
  type: "warning",
  showCancelButton: true,
  confirmButtonClass: "btn-danger",
  confirmButtonText: "Yes, get it now!",
...
function(isConfirm) {
  if (isConfirm) {
     window.location = "getNow.php?donator=<?=$email1?>&donation=<?=$type?>&item=<?=$item?>&id=<?=$id?>";
  } else {
     window.location = "availableItems.php";
  }
});
        </script>
  ...
  <button class="btn btn-outline-success my-2 my-sm-0" type="submit">Search</button>
        ...
        <br>
<p style="margin-left: 430px; color: #f56200">Only first selection will be taken into consideration, you can select something else after 1 week.</p>
  <?php
require_once 'connection.php';
...
<a href="getNow.php?id=<?=$row['id']?>&email=<?=$row['email']?>&donation=<?=$row['donation']?>&item=<?=$row['item']?>" class="btn1">Get now</a>
</div>
</div>
 <?php
}
