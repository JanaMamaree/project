...
<a href="donate.php" class="btn2">donate now</a>
...
<body>
   <?php
	...
	$rating = $_POST['rating'];
        ...
        $query1="select * from rating where email='$email'";
        $result = mysqli_query($con, $query1);
       if(mysqli_num_rows($result)== 0){
            $query="insert rating values('$email','$rating')";
            if(mysqli_query($con, $query) === FALSE) die("Could not add the new rating");
         
            echo '<br><h2 style="margin-left: 150px">Thanks for rating! '.$rating.'</h2>';
         }
         else{
             while($row = mysqli_fetch_assoc($result)){
             echo '<br><h2 style="margin-left: 150px">You already rated with '.$row['rate'].' stars' .'</h2>';
         }
         }
    }
?>
	...
	<form action="#" method="post">
            <label for="ratinginput" class="control-label">Give rating for our website:</label><br>
            <input id="ratinginput" name="rating" class="rating rating-loading" data-min="0" data-max="5" data-step="0.1" value="2"><br>
            <input style="margin-left: 100px" type="submit" name="Submit"/>
	</form>
</div>
<script>
$("#ratinginput").rating();
