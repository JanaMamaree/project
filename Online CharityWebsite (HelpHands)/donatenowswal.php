...
<div class="container">
<div class="row"> 
<div class="col-sm-6"><h1>AVAILABLE PEOPLE</h1></div>
...
<link rel="stylesheet" type="text/css" href="css/sweetalert.css">
    ...
    swal({
  title: "Are you sure?",
  text: "by confirming you are donating with <?=$item?> with id <?=$id?>",
  type: "warning",
...
function(isConfirm) {
  if (isConfirm) {
     window.location = "donatenow.php?id=<?=$id?>&email=<?=$email1?>&donation=<?=$type?>&item=<?=$item?>";
  } else {
     window.location = "availablePeople.php";
  }
});
        </script>
    ...
    <br>
    <form method="POST" align="center" class="form-inline" action="search.php">
  ...
  <button class="btn btn-outline-success my-2 my-sm-0" type="submit">Search</button>
    ...
    <br>
     <p style="margin-left: 420px; color: #f56200">When you donate to someone, our team will contact you via email with all details within 24 hours.</p>
<?php
require_once 'connection.php';
      
$getorders = "select * from aid where status='Confirmed!'";
$result = mysqli_query($con, $getorders);
//$return_array = array();
echo '<br/><br/><table class="table">' . "\r\n";
echo "<tr><th scope='col'>ID</th><th scope='col'>City</th><th scope='col'>Street</th><th scope='col'>Type of donation</th><th scope='col'>Item</th><th scope='col'>Select to donate</th></tr>";
$totalall=0;

while ($row = mysqli_fetch_assoc($result)) {
?>
<tr><td scope="row"><?=$row['id']?></td><td scope="row"><?=$row['city']?></td><td scope="row"><?=$row['street']?></td><td scope="row"><?=$row['donation']?></td><td scope="row"><?=$row['item']?></td><td scope="row"><a href="donatenowswal.php?id=<?=$row['id']?>&email=<?=$row['email']?>&donation=<?=$row['donation']?>&item=<?=$row['item']?>"><button style="background-color: #c1e2b3;" type="button">Donate</button></a></td></tr>
